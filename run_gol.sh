...
do
    ...
    do
        for k in 1 2 3
        do
            OMP_NUM_THREADS=${j} ./bin/02_gol_cpu_openmp_loop ${i} ${i} 100 0 -1 > loop_${i}x${i}_100rounds_${j}threads_trial${k}.txt
            mv ./GOL-stats.txt ./stats_loop_${i}x${i}_100rounds_${j}threads_trial${k}.txt
        done
